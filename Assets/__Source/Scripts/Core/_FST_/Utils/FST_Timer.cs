...
//
//	Description:	FST_Timer is a script extension for delaying (scheduling) methods
...
public class FST_Timer : MonoBehaviour
{
    ...
    /// <summary>
    /// returns false if this script was not added via the 'FST_Timer.In'
    ...
    /// <summary>
    /// in Awake a check is made to see if the FST_Timer component
    ...
    private void Update()
    {
        ...
        m_EventBatch = 0;
        while ((FST_Timer.m_Active.Count > 0) && m_EventBatch < MaxEventsPerFrame)
        {
            ...
            if (m_EventIterator < 0)
            {
                ...
                m_EventIterator = FST_Timer.m_Active.Count - 1;
                break;
            }

            // prevent index out of bounds
            if (m_EventIterator > FST_Timer.m_Active.Count - 1)
                m_EventIterator = FST_Timer.m_Active.Count - 1;

            // execute all due events
            if (Time.time >= FST_Timer.m_Active[m_EventIterator].DueTime ||  // time is up
                FST_Timer.m_Active[m_EventIterator].Id == 0)                 // event has been canceled ('Execute' will kill it)
                FST_Timer.m_Active[m_EventIterator].Execute();
            else
            {
                // handle pausing
                if (FST_Timer.m_Active[m_EventIterator].Paused)
                    FST_Timer.m_Active[m_EventIterator].DueTime += Time.deltaTime;
                else
                    // log lifetime
                    FST_Timer.m_Active[m_EventIterator].LifeTime += Time.deltaTime;
            }
    ...
    private static void Schedule(float time, Callback func, ArgCallback argFunc, object args, Handle timerHandle, int iterations, float interval)
    {
        ...
        {
            UnityEngine.Debug.LogError("Error: (FST_Timer) Aborted event because function is null.");
            return;
        }
        ...
        if (m_MainObject == null)
        {
            ...
            if (!m_AppQuitting)
            {
                ...
                m_MainObject.AddComponent<FST_Timer>();
        ...
        FST_Timer.m_EventCount++;
        m_NewEvent.Id = FST_Timer.m_EventCount;
        ...
        FST_Timer.m_Active.Add(m_NewEvent);
    ...
    /// </summary>
    private static void Cancel(FST_Timer.Handle handle)
    {
    ...
    public static void CancelAll()
    {
        ...
        for (int t = FST_Timer.m_Active.Count - 1; t > -1; t--)
        {
            FST_Timer.m_Active[t].Id = 0;
        }
    ...
    public static void CancelAll(string methodName)
    {
        ...
        for (int t = FST_Timer.m_Active.Count - 1; t > -1; t--)
        {
            if (FST_Timer.m_Active[t].MethodName == methodName)
                FST_Timer.m_Active[t].Id = 0;
        }
    ...
    {
        FST_Timer.m_Active.Clear();
        FST_Timer.m_Pool.Clear();
    ...
    private void OnLevelLoad(Scene scene, LoadSceneMode mode)
    {
        ...
        for (int t = FST_Timer.m_Active.Count - 1; t > -1; t--)
        {
            if (FST_Timer.m_Active[t].CancelOnLoad)
                FST_Timer.m_Active[t].Id = 0;
        }
    ...
    /// <summary>
    /// provides the custom FST_Timer editor class with debug info
    /// </summary>
    ...
    /// <summary>
    /// provides the custom FST_Timer editor class with the name of a
    ...
    /// <summary>
    /// provides the custom FST_Timer editor class with the id of a
    ...
    //					an event post initiation, schedule it with a 'FST_Timer.Handle'
    //
    /////////////////////////////////////////////////////////////////////////////////
    private class Event
    {
        ...
        /// <summary>
        /// performs internal recycling of the FST_Timer
        /// </summary>
        private void Recycle()
        {
            ...
            if (FST_Timer.m_Active.Remove(this))
                m_Pool.Add(this);
        ...
        private void Destroy()
        {
            ...
            FST_Timer.m_Active.Remove(this);
            FST_Timer.m_Pool.Remove(this);
    ...
    public class Handle
    {
        ...
        private FST_Timer.Event m_Event = null;  // timer we're pointing at
        ...
        public int Id
        {
            ...
            set
            {
                ...
                for (int t = FST_Timer.m_Active.Count - 1; t > -1; t--)
                {
                    if (FST_Timer.m_Active[t].Id == m_Id)
                    {
                        m_Event = FST_Timer.m_Active[t];
                        break;
                    }
                }
                if (m_Event == null)
