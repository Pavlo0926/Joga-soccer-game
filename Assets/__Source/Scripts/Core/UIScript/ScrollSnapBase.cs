            ...
            private void ToggleNavigationButtons(int targetScreen)
            {
                  if (UIController.Instance)
                  {
                        if (PrevButton)
                        {
                              ...
                              if (PrevButton.GetComponent<Button>().interactable == false)
                        ...
                        if (NextButton)
                        {
                              // changes are made to show and hide next button 
                              //changes as do not want to hide next button
                              NextButton.GetComponent<Button>().interactable = targetScreen < _screensContainer.transform.childCount - 1;

                              if (!NextButton.activeSelf)
                              {
                                    gameObject.GetComponent<ScrollRect>().scrollSensitivity = 0;
                              }
                              if (targetScreen == 0)
                              {
                                    gameObject.GetComponent<ScrollRect>().scrollSensitivity = 0;
                              }
                              //    Debug.Log("Target screen"+targetScreen);
                              //    Debug.Log("childCount"+_screensContainer.transform.childCount);
                              // NextButton.GetComponent<Button>().interactable = targetScreen >= 0;

                              if (NextButton.GetComponent<Button>().interactable == false)
                              {
                                    UIController.Instance.SkipTutorials.SetActive(false);
                                    NextButton.SetActive(false);
                              }
                              else
                              {
                                    NextButton.SetActive(true);
                                    UIController.Instance.SkipTutorials.SetActive(true);
                              }
