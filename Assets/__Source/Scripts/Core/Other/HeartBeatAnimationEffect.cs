...
public class HeartBeatAnimationEffect : MonoBehaviour
{
    ...
    IEnumerator AnimatePulse(GameObject _btn)
    {
        ...
        while (t <= 1.0f)
        {
            ...
            _btn.transform.localScale = new Vector3(Mathf.SmoothStep(startScaleX, endScaleX, t), Mathf.SmoothStep(startScaleY, endScaleY, t), _btn.transform.localScale.z);
            yield return 0;
        }
        ...
        if (_btn.transform.localScale.x >= endScaleX)
        {
            while (r <= 1.0f)
            {
                ...
                _btn.transform.localScale = new Vector3(Mathf.SmoothStep(endScaleX, startScaleX, r), Mathf.SmoothStep(endScaleY, startScaleY, r), _btn.transform.localScale.z);
